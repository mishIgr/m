...
impl ConnectionManager {
    ...
    pub fn start_server(&mut self, server_id: &str) -> Result<()> {
        m_core::log_info!("connection_manager: start_server server={}", server_id);
        let server = self.store.load_server(server_id)?;
        if server.state != NodeState::Enabled {
            m_core::log_warn!("connection_manager: start_server server={} not enabled state={}", server_id, server.state);
    ...
    fn start_server_inner(&mut self, server_id: &str, address: &str, shared_key_bytes: &[u8]) {
        if let Some(old) = self.cancels.remove(server_id) {
            m_core::log_debug!("connection_manager: cancelling old task for server={}", server_id);
            old.cancel();
        }
        ...
        let chats = match self.store.list_enabled_chats_for_server(server_id) {
            Ok(c) => c,
            Err(e) => {
                m_core::log_warn!("connection_manager: list_enabled_chats_for_server failed server={}: {}", server_id, e);
                vec![]
            }
        };

        m_core::log_info!("connection_manager: spawning live task server={} address={} chats={}", server_id, address, chats.len());
    ...
    pub fn stop_server(&mut self, server_id: &str) {
        if let Some(cancel) = self.cancels.remove(server_id) {
            m_core::log_info!("connection_manager: stop_server server={}", server_id);
            cancel.cancel();
        } else {
            m_core::log_debug!("connection_manager: stop_server server={} was not running", server_id);
        }
    }
    ...
    pub fn restart_server(&mut self, server_id: &str) -> Result<()> {
        m_core::log_info!("connection_manager: restart_server server={}", server_id);
    ...
    pub fn handle_unavailable_server(&mut self, server_id: &str) {
        ...
        if let Err(e) = self.store.set_server_state(server_id, NodeState::Unavailable) {
            m_core::log_error!("connection_manager: handle_unavailable_server persist failed server={}: {}", server_id, e);
        }
    }
    ...
    pub fn handle_disconnected(&mut self, server_id: &str) {
        ...
        self.cancels.remove(server_id);
        if let Err(e) = self.store.set_server_state(server_id, NodeState::Unavailable) {
            m_core::log_error!("connection_manager: handle_disconnected persist failed server={}: {}", server_id, e);
        }
    }
}
