...
async fn main() -> anyhow::Result<()> {
    ...
    let dispatcher = Dispatcher::new(config.delivery, config.performance.dedup_cache_size, chats_table)?;
    ...
    let service = MessengerService::new(dispatcher, cipher, admin_key, config.performance.channel_buffer_size);
    service.spawn_retry_loop(std::time::Duration::from_secs(config.delivery.retry_interval_secs));
